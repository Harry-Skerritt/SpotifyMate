...
void captiveGraphics(const char *ssid) {
    ...
    tft.setTextDatum(MC_DATUM);
    tft.drawString("to your network. Once Connected the device will restart!", (tft.width() / 2), 280);
...
void spotifyConnectScreen(String hostname) {
    ...
    tft.setTextDatum(MC_DATUM);
    tft.drawString("SpotifyMate is now on your network!", (tft.width() /2), 90);

    tft.setTextDatum(MC_DATUM);
    tft.drawString("In a web browser, visit:", (tft.width() / 2), 120);

    tft.setTextColor(toInt(30, 215, 96));
    tft.setTextSize(4);
    tft.setTextDatum(MC_DATUM);
    tft.drawString(hostname, tft.width()/2, (tft.height()/2) + 20);

    tft.setTextColor(toInt(255, 255, 255));
    tft.setTextSize(2);

    tft.setTextDatum(MC_DATUM);
    tft.drawString("and click 'Connect Spotify' to link", (tft.width() / 2), 250);

    tft.setTextDatum(MC_DATUM);
    tft.drawString("SpotifyMate with your Spotify account!", (tft.width() / 2), 280);

}

bool displayOutput(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t *bitmap)
{
    tft.fillScreen(toInt(18, 18, 18)); //Clear Screen - DEBUG
    // Stop further decoding as image is running off bottom of screen
    if (y >= tft.height()) //Hardcoded version of tft.height();
        return 0;
